...
- `it_retrievalActions.ts`: retrieval settings and cache-maintenance use-cases for Webview commands (includes numeric clamp guards for hostile payloads).
- `it_templateActions.ts`: template/secrets/tokens use-cases for Webview commands.
- `it_workspaceActions.ts`: workspace and sessions directory selection use-cases (enforces in-workspace path constraints).
...
- `it_resultActions.ts`: result-view actions (open file/analyze/cancel) use-cases for Webview commands (validates analyze request shape before execution).
- `it_analysisFlow.ts`: end-to-end analysis run use-case; session state is delegated to `application/services/it_analysisSessionState.ts`, run deps are delegated to `application/services/it_analysisRunConfig.ts`, host contracts are centralized in `application/services/it_analysisHostPorts.ts`, and cancel/error semantics are delegated to `application/services/it_analysisErrors.ts`.
