...
## 目录与关键文件
- `src/interviewTrainer/handlers/it_webviewHandlers.ts`：总入口注册
- `it_webviewCoreHandlers.ts`：分析/取消/保存
...
- `it_webviewResultHandlers.ts`：结果处理与回写
- `it_webviewTestHandlers.ts`：模板测试/日志

## 关键调用链
- `webview/src/messenger.ts` → `WebviewProtocol` → `it_webviewHandlers.ts`
- 分析：`it_webviewCoreHandlers.ts` → `core/it_analysisFlow.ts`
- 模板测试：`it_webviewTestHandlers.ts` → `api/it_templateExecutor.ts`
