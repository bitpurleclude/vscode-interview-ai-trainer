...
- `src/interviewTrainer/infra/api/it_llm.ts`：LLM 调用入口（provider 选择、stream、模板）。
- `src/interviewTrainer/infra/api/it_llmHelpers.ts`：nonce/模板变量/响应解析等通用逻辑。
- `src/interviewTrainer/infra/api/it_llmStream.ts`：SSE 增量解析与拼接。
- `src/interviewTrainer/infra/api/it_llmOpenAi.ts`：OpenAI-compatible 适配。
- `src/interviewTrainer/infra/api/it_llmDoubao.ts`：Doubao 适配。
- `src/interviewTrainer/infra/api/it_llmTemplate.ts`：模板运行封装。
- `src/interviewTrainer/infra/api/it_embedding.ts`：通用 Embedding 调用入口。
...
- 新增模板时先验证 `it_templateExecutor` 并查看日志。
