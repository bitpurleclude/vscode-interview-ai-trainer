...
## 目录与关键文件
- `src/interviewTrainer/api/it_apiConfig.ts`：配置结构与默认值、用户目录定位
- `src/interviewTrainer/api/it_configService.ts`：配置加载/合并/保存（含模板配置）
- `src/interviewTrainer/api/it_templateExecutor.ts`：模板渲染、请求执行、模板变量解析
- `src/interviewTrainer/api/it_requestBuilder.ts`：将模板与变量转为请求体
- `src/interviewTrainer/api/it_llm.ts` / `it_embedding.ts`：通用 LLM/Embedding 调用入口
- `src/interviewTrainer/api/it_baidu.ts` / `it_qianfan.ts` / `it_volc_asr.ts`：厂商适配
- `src/interviewTrainer/api/it_llmTypes.ts`：LLM 配置类型
- `src/interviewTrainer/api/it_toolsPresets.ts` + `toolsPresets/presets/*`：工具预设
...
- 分析流程模板：`core/analyze/flow.ts` → `it_resolveBindingTemplate()` → `it_buildTemplateLlmConfig()`
