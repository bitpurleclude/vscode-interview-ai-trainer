...
const InterviewTrainer: React.FC = () => {
  ...
  const { result: initialState } = useIdeMessengerRequest(
    "it/getState",
    undefined,
  );
  const { result: initialConfig } = useIdeMessengerRequest(
    "it/getConfig",
    undefined,
  );
  ...
  const handleAnalyze = async () => {
    ...
    const response = await ideMessenger.request("it/analyzeAudio", payload);
  ...
  const handleOpenReport = async () => {
    ...
    await ideMessenger.request("openFile", { path: analysisResult.reportPath });
  };

  const handleLoadHistory = async () => {
    const response = await ideMessenger.request("it/listHistory", {
      limit: 30,
    });
    if (response.status === "success") {
    ...
    return (
      <div className="it-steps">
        {steps.map((step) => (
          <div
            key={step.id}
            className={`it-step it-step--${step.status}`}
          >
  ...
  return (
    <div className="it-root">
      <div className="it-header">
        ...
        <div className="it-actions">
          ...
          <button
            className="it-button"
            disabled={uiLocked}
            onClick={() => ideMessenger.request("it/openSettings", undefined)}
          >
            设置
          </button>
        </div>
      ...
      <div className="it-status">
        ...
        {itState.lastError && (
          <span className="it-status__error">
            {itState.lastError.reason}
          </span>
        )}
      </div>
